...
///
/// Assumes delays to be specified as `std::time::Duration`.
/// Puts the current thread to sleep between retries.
/// For a more generic implementation, use `retry_with_delay_strategy`.
...
/// Retries the given `operation` until its outcome needs no more retries
/// or the `retry_strategy` states that it is exhausted.
///
/// Assumes delays to be specified as `std::time::Duration`.
/// Tells the tokio runtime to sleep between retries.
/// For a more generic implementation, use `retry_async_with_delay_strategy`.
#[cfg(feature = "async")]
